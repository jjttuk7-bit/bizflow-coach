...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ 
        ...
        return (
             <div key={index} className={`flex items-end gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                ...
                {!isUser && typeof msg.author === 'object' && msg.author && (
                    <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-white ${msg.author.classes.bg}`}>
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-8 bg-white rounded-xl shadow-lg animate-fade-in space-y-6 relative">
            ...
            {charts && charts.length > 0 && (
                <div className="space-y-6 p-4 border border-gray-200 rounded-lg bg-slate-50">
            ...
            <div className="p-6 bg-slate-50 rounded-lg min-h-[300px] overflow-y-auto max-h-[60vh]">
                {isLoading && conversation.length === 0 && (
                     <div className="flex flex-col items-center justify-center h-full">
                        <SpinnerIcon className="w-12 h-12 animate-spin text-indigo-600" />
                        <p className="mt-4 text-lg text-gray-700">AI가 분석 중입니다...</p>
                    </div>
                )}
                {error && (
            ...
            {onFollowUpQuery && !isLoading && (result || (conversation && conversation.length > 0)) && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                    <label className="block text-md font-bold text-gray-700 mb-2">이 리포트에 대해 추가 질문하기</label>
                    <div className="relative">
                        <textarea
                            ...
                            className="w-full p-3 pr-12 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 resize-none bg-white"
                            rows={1}
            ...
            {showProfitGoalInput && onProfitGoalSubmit && (
                 <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex flex-col sm:flex-row items-center gap-4">
                    <div className="flex-grow">
                        ...
                        <input
                            ...
                            className="mt-1 block w-full p-2 bg-white border border-yellow-300 rounded-md shadow-sm focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm"
                            placeholder="예: 500만원"
                        />
                    </div>
                    <button 
                        ...
                        className="w-full sm:w-auto flex-shrink-0 flex items-center justify-center gap-2 px-4 py-2 bg-yellow-500 text-white font-semibold rounded-md hover:bg-yellow-600 disabled:bg-gray-400"
                    >
            ...
            <div className="flex flex-col sm:flex-row justify-end items-center gap-4 pt-4">
                {showSimulateButton && onSimulate && (
                    <button
                        onClick={onSimulate}
                        className="w-full sm:w-auto px-6 py-2 border border-indigo-600 text-indigo-600 font-semibold rounded-md shadow-sm hover:bg-indigo-50"
                    >
                        3일 후 재고 상황 시뮬레이션하기
                    </button>
                )}
                <button
                    onClick={onBack}
                    className="w-full sm:w-auto px-6 py-2 bg-indigo-600 text-white font-semibold rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                >
